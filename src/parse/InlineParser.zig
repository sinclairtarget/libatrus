...
pub fn parse(
    ...
    for (0..util.safety.loop_bound) |_| { // could hit if we forget to consume tokens
        ...
        if (try self.parseFullReferenceImage(alloc, scratch)) |image| {
            try nodes.append(image);
            continue;
        }

        if (try self.parseCollapsedReferenceImage(alloc, scratch)) |image| {
            try nodes.append(image);
            continue;
        }

        if (try self.parseShortcutReferenceImage(alloc, scratch)) |image| {
            try nodes.append(image);
            continue;
        }
            ...
            const close_token_type = switch (token.token_type) {
                ...
                .r_delim_underscore, .lr_delim_underscore => {
                    ...
                    const underscore_open_token = (
                        opts.maybe_underscore_open_token orelse
                        break :blk false
                    );
                    // zig fmt: on
                    if (isValidBySumOfLengthsRule(
                        underscore_open_token,
                        token,
                    )) {
        ...
        const text_content = blk: {
            ...
            swtch: switch (token.token_type) {
                ...
                .r_delim_underscore, .lr_delim_underscore => {
                    ...
                    // zig fmt: off
                    const underscore_open_token = (
                        opts.maybe_underscore_open_token orelse break :swtch
                    );
                    // zig fmt: on
                    if (isValidBySumOfLengthsRule(
                        underscore_open_token,
            ...
            const close_token_type = switch (token.token_type) {
                ...
                .r_delim_star, .lr_delim_star => {
                    ...
                    const star_open_token = (
                        opts.maybe_star_open_token orelse break :blk false
                    );
                    // zig fmt: on
...
fn isValidBySumOfLengthsRule(open: InlineToken, close: InlineToken) bool {
    if (open.token_type == .lr_delim_star or
        close.token_type == .lr_delim_star)
    {
        const sum_of_len = open.context.delim_star.run_len +
            close.context.delim_star.run_len;
        if (sum_of_len % 3 == 0) {
            return open.context.delim_star.run_len % 3 == 0 and
                close.context.delim_star.run_len % 3 == 0;
        }
    }

    if (open.token_type == .lr_delim_underscore or
        close.token_type == .lr_delim_underscore)
    {
        const sum_of_len = open.context.delim_underscore.run_len +
            close.context.delim_underscore.run_len;
        if (sum_of_len % 3 == 0) {
            return open.context.delim_underscore.run_len % 3 == 0 and
                close.context.delim_underscore.run_len % 3 == 0;
        }
    }
...
fn parseInlineLink(
    ...
    // handle link text
    const link_text_nodes = try self.parseLinkText(alloc, scratch) orelse
        return null;
    defer if (!did_parse) {
...
fn parseFullReferenceLink(
    ...
    // handle link label
    const scanned_link_label = try self.scanLinkLabel(scratch) orelse
        return null;
...
fn parseCollapsedReferenceLink(
    ...
    self.backtrack(checkpoint_index);
    const inline_nodes = try self.parseLinkLabel(alloc, scratch) orelse
        return null;
    defer if (!did_parse) {
...
fn parseInlineImage(
    ...
    const img_desc_nodes = try self.parseImageDescription(alloc, scratch) orelse
        return null;
    defer {
    ...
    const whitespace = try self.scanSeparatingWhitespace(scratch) orelse
        return null;
    const title = blk: {
        // link title, if present, must be separated from destination by
        // whitespace
        ...
        .normal => {
            ...
            switch (token.token_type) {
                ...
                .lr_delim_underscore => {
                    ...
                    if (token.context.delim_underscore.preceded_by_punct or
...
test "underscore emphasis interior sum of lengths rule" {
    ...
    try testing.expectEqual(1, nodes.len);
    try testing.expectEqual(ast.NodeType.emphasis, @as(ast.NodeType, nodes[0].*));
    try testing.expectEqual(1, nodes[0].emphasis.children.len);
...
test "interleaved emphasis" {
    ...
    try testing.expectEqual(ast.NodeType.text, @as(ast.NodeType, nodes[1].*));
    try testing.expectEqualStrings(
        "_",
        nodes[1].text.value,
    );
    ...
    try testing.expectEqual(ast.NodeType.emphasis, @as(ast.NodeType, nodes2[0].*));
    try testing.expectEqual(1, nodes2[0].emphasis.children.len);
    try testing.expectEqual(
        ast.NodeType.text,
        @as(ast.NodeType, nodes2[0].emphasis.children[0].*),
    );
    try testing.expectEqualStrings(
        "*bar",
        nodes2[0].emphasis.children[0].text.value,
    );

    try testing.expectEqual(ast.NodeType.text, @as(ast.NodeType, nodes2[1].*));
    try testing.expectEqualStrings(
        "*",
        nodes2[1].text.value,
    );
...
test "nested interleaved emphasis" {
    ...
    try testing.expectEqual(ast.NodeType.emphasis, @as(ast.NodeType, nodes[1].*));
    try testing.expectEqual(1, nodes[1].emphasis.children.len);
    try testing.expectEqual(
        ast.NodeType.text,
        @as(ast.NodeType, nodes[1].emphasis.children[0].*),
    );
...
test "interleaved strong" {
    ...
    try testing.expectEqual(ast.NodeType.strong, @as(ast.NodeType, nodes2[0].*));
    try testing.expectEqual(1, nodes2[0].strong.children.len);
    try testing.expectEqual(
        ast.NodeType.text,
        @as(ast.NodeType, nodes2[0].strong.children[0].*),
    );
    try testing.expectEqualStrings(
        "**bar",
        nodes2[0].strong.children[0].text.value,
    );
...
test "codespan escaped backtick" {
    ...
    try testing.expectEqual(ast.NodeType.inline_code, @as(ast.NodeType, nodes[0].*));
    try testing.expectEqualStrings(
        "foo\\",
        nodes[0].inline_code.value,
    );
...
test "inline link nested emphasis with nonsignificant brackets" {
    ...
    try testing.expectEqualStrings(
        "",
        link_node.link.title,
    );

    try testing.expectEqual(3, link_node.link.children.len);
    ...
    {
        const emphasis_node = link_node.link.children[0];
        ...
        try testing.expectEqualStrings(
            ...
            emphasis_node.emphasis.children[0].text.value,
        );
    }
    ...
    try testing.expectEqual(
        ast.NodeType.text,
        @as(ast.NodeType, link_node.link.children[1].*),
    );
    try testing.expectEqualStrings(
        "[",
        link_node.link.children[1].text.value,
    );

    // second emphasis node
    try testing.expectEqual(
        ast.NodeType.emphasis,
        @as(ast.NodeType, link_node.link.children[2].*),
    );
    {
        const emphasis_node = link_node.link.children[2];
        try testing.expectEqual(1, emphasis_node.emphasis.children.len);
        try testing.expectEqual(
            ast.NodeType.text,
            @as(ast.NodeType, emphasis_node.emphasis.children[0].*),
...
test "image" {
    ...
    try testing.expectEqualStrings(
        "foo",
        image_node.image.alt,
    );
    try testing.expectEqualStrings(
        "/bar",
        image_node.image.url,
    );
    try testing.expectEqualStrings(
        "bim",
        image_node.image.title,
    );
}
...
test "reference image precedence" {
    ...
    try testing.expectEqual(ast.NodeType.link, @as(ast.NodeType, nodes[1].*));
    try testing.expectEqualStrings(
        "/url1",
        nodes[1].link.url,
    );
...
test "html open tag" {
    ...
    try testing.expectEqual(1, nodes.len);
    try testing.expectEqual(ast.NodeType.html, @as(ast.NodeType, nodes[0].*));
    try testing.expectEqualStrings(
        "<span>",
        nodes[0].html.value,
    );
...
test "html open tag multiple" {
    ...
    try testing.expectEqual(ast.NodeType.html, @as(ast.NodeType, nodes[1].*));
    try testing.expectEqualStrings(
        "<bar />",
        nodes[1].html.value,
    );
...
test "html within emphasis" {
    ...
    try testing.expectEqual(1, nodes.len);
    try testing.expectEqual(ast.NodeType.emphasis, @as(ast.NodeType, nodes[0].*));
    try testing.expectEqual(2, nodes[0].emphasis.children.len);
    try testing.expectEqual(
        ast.NodeType.html,
        @as(ast.NodeType, nodes[0].emphasis.children[0].*),
    );
    try testing.expectEqual(
        ast.NodeType.html,
        @as(ast.NodeType, nodes[0].emphasis.children[1].*),
    );
}
...
test "html within strong" {
    ...
    try testing.expectEqual(1, nodes.len);
    try testing.expectEqual(ast.NodeType.strong, @as(ast.NodeType, nodes[0].*));
    try testing.expectEqual(2, nodes[0].strong.children.len);
    try testing.expectEqual(
        ast.NodeType.html,
        @as(ast.NodeType, nodes[0].strong.children[0].*),
    );
    try testing.expectEqual(
        ast.NodeType.html,
        @as(ast.NodeType, nodes[0].strong.children[1].*),
    );
}
...
test "html within link" {
    ...
    try testing.expectEqual(2, nodes[0].link.children.len);
    try testing.expectEqual(
        ast.NodeType.html,
        @as(ast.NodeType, nodes[0].link.children[0].*),
    );
    try testing.expectEqual(
        ast.NodeType.html,
        @as(ast.NodeType, nodes[0].link.children[1].*),
    );
}
...
test "unknown myst role" {
    ...
    try testing.expectEqual(1, nodes.len);
    try testing.expectEqual(
        ast.NodeType.myst_role,
        @as(ast.NodeType, nodes[0].*),
    );
    try testing.expectEqual(0, nodes[0].myst_role.children.len);
    try testing.expectEqualStrings(
        "foo",
        nodes[0].myst_role.name,
    );
    try testing.expectEqualStrings(
        "bar",
        nodes[0].myst_role.value,
    );
}
...
test "myst role with invalid name" {
    ...
    try testing.expectEqual(1, nodes.len);
    try testing.expectEqual(
        ast.NodeType.myst_role_error,
        @as(ast.NodeType, nodes[0].*),
    );
    try testing.expectEqualStrings(
        "baz",
        nodes[0].myst_role_error.value,
    );
}
...
test "myst role with whitespace around name" {
    ...
    try testing.expectEqual(1, nodes.len);
    try testing.expectEqual(ast.NodeType.myst_role, @as(ast.NodeType, nodes[0].*));
    try testing.expectEqual(0, nodes[0].myst_role.children.len);
    try testing.expectEqualStrings(
        "foo",
        nodes[0].myst_role.name,
